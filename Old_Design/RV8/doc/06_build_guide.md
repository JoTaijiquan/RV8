...
**Chips**: Crystal oscillator module (3.5 MHz breadboard / 10 MHz PCB)

**Build**: Wire oscillator output to CLK rail. Add /RST with 10K pull-up + 100nF debounce + pushbutton to GND.

**Test**: Scope shows 3.5 MHz on CLK rail, clean /RST low when button pressed.

**You now have**: free-running clock + clean reset

**Note**: Single-step is provided by the Trainer board (not on CPU board).
...
- Use single-step mode (via Trainer board) for debugging, free-run when it works
...
# แลป 1: Clock + Reset (Crystal Oscillator)

## วงจร Clock

| จุด | ต่อกับ |
|-----|--------|
| Crystal Oscillator 3.5 MHz | output → **CLK rail** (ไปทุกชิปที่ต้องการ clock) |
...
## ผลลัพธ์: ได้สัญญาณ CLK (free-running) และ /RST

**หมายเหตุ**: Single-step อยู่บน Trainer board (ไม่ได้อยู่บน CPU board)
...
## ผลลัพธ์: Single-step (ผ่าน Trainer board) แล้ว LED แสดงค่าจาก ROM ตามลำดับ
...
| จาก | → ไป | สัญญาณ |
|-----|------|--------|
| a0 [7:0] + operand [7:0] | 74HC08 inputs | AND operation |
| a0 [7:0] + operand [7:0] | 74HC32 inputs | OR operation |
| Adder output / AND output / OR output / XOR output | 74HC157 mux inputs | result select |
| alu_op [1:0] (from decode) | 74HC157 S pin | เลือกผลลัพธ์ |
| Mux output | a0 D input | ผลลัพธ์ ALU |

### Shift (ไม่ต้องชิปเพิ่ม):
- SHL: ต่อ a0[6:0] → result[7:1], result[0]=0
- SHR: ต่อ a0[7:1] → result[6:0], result[7]=0
- ROL/ROR: ต่อผ่าน carry flag
...
ไม่ต้องชิปใหม่ — เพิ่มสายต่อ:

### ต่อสาย:
| จาก | → ไป | สัญญาณ |
|-----|------|--------|
| U9 (sp) output | Address mux low byte (S=10) | stack address low |
| $30 (hardwire) | Address mux high byte (S=10) | stack address high = $30xx |
| sp_dec signal | U9 CLK gate | SP-1 ก่อน PUSH |
| sp_inc signal | U9 CLK gate | SP+1 หลัง POP |
| PC high/low | data_out mux | JAL push PC |
| State machine | extend S2→S3→S4 | multi-cycle ops |

### ทดสอบ:
- PUSH a0 → POP a0: ค่าต้องเหมือนเดิม
- JAL addr → RET: กลับมาทำงานต่อถูกที่
...
| จาก | → ไป | สัญญาณ |
|-----|------|--------|
| opcode [2:0] | wr_sel decode | เลือก register ที่จะเขียน |
| wr_sel output | CLK gate ของ U7-U10 | เปิด write ทีละตัว |
| $00 (hardwire) | Address mux high (mode=ZP) | zero-page: {$00, imm} |
| pg register (U10) | Address mux high (mode=PG) | page: {pg, imm} |
| sp + imm (adder) | Address mux low (mode=SP+) | stack-relative |
| Register read mux | ALU B input | MOV path |

### Constant generator (register c0):
- opcode[1:0] = 00 → 0x00
- opcode[1:0] = 01 → 0x01
- opcode[1:0] = 10 → 0xFF
- opcode[1:0] = 11 → 0x80
...
เพิ่ม: 74HC74 (NMI edge detect — U24 ในบาง layout ใช้ FF ว่างจาก U20/U21)

### ต่อสาย:
| จาก | → ไป | สัญญาณ |
|-----|------|--------|
| /NMI pin (external) | 74HC74 CLK (inverted) | จับขอบขาลง |
| +5V | 74HC74 D | พร้อม latch เสมอ |
| 74HC74 Q | nmi_pending → control | แจ้ง CPU |
| int_ack (from control) | 74HC74 /CLR | CPU เคลียร์หลัง handle |
| /IRQ pin AND IE flag | irq_active → control | maskable interrupt |
| Vector address ($FFFA-$FFFF) | Address mux (mode=vector) | อ่าน vector |
| Skip flag (1-bit FF) | write_enable gate | ระงับ 1 คำสั่ง |

### ลำดับ Interrupt:
1. ตรวจ nmi_pending หรือ irq_active
2. PUSH PCH → PUSH PCL → PUSH flags
3. อ่าน vector low → อ่าน vector high
4. โหลด PC ← vector → fetch จากที่ใหม่

### ลำดับ RTI:
1. POP flags → POP PCL → POP PCH → โหลด PC

## ผลลัพธ์: **ครบ 68 คำสั่ง! CPU สมบูรณ์!** 🎉

---

# สรุปชิปทั้งหมด (26 ตัว)
...
| U24 | 74HC138 | 3-to-8 Line Decoder/Demux | Address decode (ROM/RAM/I/O) |
...
- ใช้ Trainer board (STEP mode) ตอนดีบัก, ปล่อย free-run ตอนทำงานจริง
