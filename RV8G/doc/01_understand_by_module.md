# RV8-G — Understand by Module (How Chips Become a CPU)

**25 logic chips. No microcode. Instruction byte directly controls hardware.**

---

## How RV8-G differs from RV8:
...
## 5 Modules (RV8-G is simpler than RV8!)
...
**RV8-G cannot do LB (load byte from memory) in a clean way** because:
...
RV8-G has **no microcode** to compute PC+1. So PC must auto-increment itself. 74HC161 is a counter — it increments every clock automatically. No ALU needed!
...
**Honest RV8-G: 27 logic chips** (same as RV8, after fixing LB issue).
...
**RV8-G is still faster (2× fewer cycles) but loses relative branch and logic ops.**

---

# ภาษาไทย — RV8-G ทำงานอย่างไร

## ความแตกต่างจาก RV8:

```
RV8:   สมอง (Flash) สั่งทุกอย่าง ทีละขั้น (4-8 ขั้นต่อคำสั่ง)
RV8-G: คำสั่งเอง สั่งตรง ไม่ต้องมีสมอง (2 ขั้นต่อคำสั่ง)
```
...
## สรุป: RV8-G ใช้ชิปเท่า RV8 (27 ตัว) แต่เร็วกว่า 2 เท่า แลกกับความสามารถที่น้อยกว่า
