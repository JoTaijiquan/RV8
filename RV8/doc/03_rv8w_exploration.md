...
```
Byte 0: [class(2)][op(3)][rd(3)]        — same as RV8!
Byte 1: [rs(3)][imm5(5)] or [imm8]      — same as RV8!
```

This is... exactly the current RV8 encoding. The difference is HOW we use it:
...
### Remaining issues:
1. **ALU result → register D**: All register D inputs must connect to ALU output (parallel, always). Only the CLK'd register captures. ✅ (same as RV8)
2. **Memory address**: Need register value on address bus for LB/SB. Needs 2nd cycle + address latch (same as RV8). +1 or +2 chips for address latches.
...
**RV8 with microcode remains the best RISC-V style option at 27-29 chips.**
